...
def transform(aug, image, anns):
    ...
    for i in poly:
        for j in i:
            lst.append(Keypoint(x=j[0], y=j[1]))

    kps = KeypointsOnImage(lst, shape=image.shape)
    ...
    for i in range(len(kps.keypoints)//4):
        k=[]
        for j in range(0+index*4,4+index*4):
            k.append((min(max(0, kps_aug.keypoints[j].x),
                     image_aug.shape[1] - 1), min(max(0, kps_aug.keypoints[j].y), image_aug.shape[0] - 1)))
        new_poly.append(k)
        index+=1

    for index, i in enumerate(new_poly):
        new_ann = {'poly': i, 'text': anns[index]['text']}
        new_anns.append(new_ann)
    return image_aug, new_anns
...
def compute_distance(xs, ys, point_1, point_2):
    ...
    cosin = (square_distance - square_distance_1 - square_distance_2) / \
            (2 * np.sqrt(square_distance_1 * square_distance_2)+ep)
...
def draw_thresh_map(polygon, canvas, mask, shrink_ratio=0.4):
    ...
    canvas[ymin_valid:ymax_valid + 1, xmin_valid:xmax_valid + 1] = np.fmax(
        1 - distance_map[ymin_valid - ymin:ymax_valid - ymax + height,
                         xmin_valid - xmin:xmax_valid - xmax + width],canvas[ymin_valid:ymax_valid + 1, xmin_valid:xmax_valid + 1])
    return canvas,mask